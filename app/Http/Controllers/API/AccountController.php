...
class AccountController extends Controller
{
    public function client($id){
        ...
        if ($response['success'] == true) {
            ...
            return view('backend.account.client',compact('transaction','id_user','avatar','role_name','city','address','phone_number','join_date','email','lastname','firstname','current_balance_cdf','current_balance_usd','saving_balance_cdf','saving_balance_usd','saving_wallet_code','current_wallet_code','depot'));
    ...
    public function client_depot($id){
        ...
        return view('backend.deposit.create_id', compact('user','users','balance_cdf','balance_usd','acnumber','solde_cdf','solde_usd','transaction'));
    }
    public function profilUser(){
        ...
        if ($response['success'] == true) {
            ...
            return view('admin.account.profil_user',compact('transaction','id_user','avatar','role_name','city','address','phone_number','join_date','email','lastname','firstname','acnumber_1','acnumber_2','account_1','account_2'));
    ...
    public function client_retrait($id){
        ...
        $balance_usd = $initialize->getCustomerBalance($customer_number, 'USD', $compte);
        $acnumber = $initialize->getAccountNumber($customer_number, $compte);
        return view('backend.withdrawal.emala.create_id', compact('user','balance_cdf','balance_usd','acnumber'));
    }
    public function client_transfert($id){
        $user_id =  Crypt::decrypt($id);
        $user = DB::connection('mysql2')->table('users')->where('id',$user_id)->first();
        $users = DB::connection('mysql2')->table('users')->where('id',$user_id)->get();
        return view('backend.transfer.emala.create_id', compact('users','user'));
    }
    public function client_mobile($id){
        $user_id =  Crypt::decrypt($id);
        $user = DB::connection('mysql2')->table('users')->where('id',$user_id)->first();
        return view('backend.withdrawal.mobile.create_id', compact('user'));
    }
    public function transfert_compte_compte($id){
        ...
        return view('backend.transfer.emala.compte_compte', compact('users','user','balance_cdf','balance_usd','acnumber'));
    ...
    public function retrait_compte_epargne($id){
        ...
        return view('backend.withdrawal.emala.saving_id', compact('user','balance_cdf','balance_usd','acnumber'));
