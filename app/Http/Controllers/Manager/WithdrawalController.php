...
use App\Http\Controllers\API\CheckTransactionAPI;
use App\Http\Controllers\API\Initialize;
use App\Http\Controllers\API\WithdrawalAPI;
...
class WithdrawalController extends Controller
{
    public function transfert_cash(){
        if (Auth::user()->role_name == "Manager") {
            return view('manager.withdrawal.emala.transfert');
        }
        elseif (Auth::user()->role_name == "Cashier") {
            return view('cashier.withdrawal.emala.transfert');
        } 
    }
    public function compte_cash(){
        if (Auth::user()->role_name == "Manager") {
            return view('manager.withdrawal.emala.compte');
        }
        elseif (Auth::user()->role_name == "Cashier") {
            return view('cashier.withdrawal.emala.compte');
        }
    }
    ...
    public function retrait_compte_epargne(Request $request){
        ...
        if ($authorization['success'] == false) {
            Alert::error('Caisse Fermée', 'Veuillez ouvrir votre caisse avant d\'effectuer cette opération.');
            // return redirect()->route('cashier.account.retrait_compte_epargne');
            return redirect()->back();
        }
        else {
            if ($verifyCustomer['success'] == false) {
